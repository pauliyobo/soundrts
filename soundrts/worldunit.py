...
class Creature(Entity):
    ...
    def _future_coords(self, rotation, target_d):
        d = old_div(self.actual_speed * VIRTUAL_TIME_INTERVAL, 1000)
        if rotation == 0:
            d = min(d, target_d) # stop before colliding target
        a = self.o + rotation
        x = self.x + old_div(d * int_cos_1000(a), 1000)
        y = self.y + old_div(d * int_sin_1000(a), 1000)
        return x, y
    ...
    def heal_nearby_units(self):
        ...
        hp = old_div(self.heal_level * PRECISION, 25)
    ...
    def harm_nearby_units(self):
        ...
        hp = old_div(self.harm_level * PRECISION, 25)
        units = self.world.get_objects2(self.x, self.y, 6 * PRECISION,
    ...
    def chance_to_hit(self, target):
        ...
        if not self.is_melee:
            result = old_div(result * (100 - target.place.terrain_cover[0 if target.airground_type != "air" else 1]), 100)
        return result
    ...
    def _delta(self, total, percentage):
        ...
        try:
            delta = int(old_div(old_div(total * percentage * VIRTUAL_TIME_INTERVAL, self.time_cost), 100))
        except ZeroDivisionError:
            delta = int(old_div(total * percentage * VIRTUAL_TIME_INTERVAL, 100))
...
class BuildingSite(_Building):
    ...
    def __init__(self, player, place, x, y, building_type):
        ...
        self._starting_hp = old_div(building_type.hp_max * 5, 100)
        self.hp = self._starting_hp
        self.timer = old_div(building_type.time_cost, VIRTUAL_TIME_INTERVAL)
